...
import OstaraBot.modules.sql.feds_sql as sql
from OstaraBot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                       dispatcher)
...
from OstaraBot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                       extract_user,
                                                       extract_user_fban)
from OstaraBot.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @OstaraBotSupport if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @OstaraBotSupport should this problem persist!"
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        fed_name = info['fname']
        #https://t.me/OstaraBotSupport/41606 // https://t.me/OstaraBotSupport/41619
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @OstaraBotSupport."
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @OstaraBotSupport !"
            )
            return
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "ostarabot_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="ostarabot_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "ostarabot_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="ostarabot_fbanned_users.csv",
