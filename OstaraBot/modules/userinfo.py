...
from OstaraBot import (DEV_USERS, OWNER_ID, SUDO_USERS, SUPPORT_USERS,
                       TIGER_USERS, WHITELIST_USERS, INFOPIC, dispatcher, sw)
from OstaraBot.__main__ import STATS, TOKEN, USER_INFO
import OstaraBot.modules.sql.userinfo_sql as sql
from OstaraBot.modules.disable import DisableAbleCommandHandler
from OstaraBot.modules.sql.global_bans_sql import is_user_gbanned
from OstaraBot.modules.sql.afk_sql import is_afk, check_afk_status
from OstaraBot.modules.sql.users_sql import get_user_num_chats
from OstaraBot.modules.sql.feds_sql import get_user_fbanlist
...
from OstaraBot import telethn as SaitamaTelethonClient, TIGER_USERS, SUDO_USERS, SUPPORT_USERS
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        text += "\n\nThe Disaster level of this person is 'God - My Creator'."
    ...
    if disaster_level_present:
        text += ' [<a href="https://t.me/OstaraBotSupport/155">?</a>]'.format(
            bot.username)
...
def about_me(update: Update, context: CallbackContext):
    ...
    if info:
        ...
        update.effective_message.reply_text(
            "There isn't one, use /setme to set one.")
